...
**Mass**: ``m`` = $(@bind m Slider(0.0u"GeV/c^2":0.01u"GeV/c^2":10.0u"GeV/c^2", default=0.51u"GeV/c^2", show_value=true))

#### Electric Field

``E_x`` = $(@bind Ex Slider(0.0u"V/m":0.1u"V/m":100.0u"V/m", default=0.0u"V/m", show_value=true))
...
``t_{\text{max}}`` = $(@bind tmax Slider(0.0u"ns":1.0u"ns":500.0u"ns", default=10.0u"ns", show_value=true))

``Δt`` = $(@bind dt Slider(0.0u"ns":0.01u"ns":2.0u"ns", default=0.01u"ns", show_value=true))
"""
...
function euler(f, r0, par, tmax, dt)
    ...
    for t in ts
		...
		r .= r .+ dt * f(r, par)
		r[5] = sqrt(c_1^2*(r[6]^2 + r[7]^2 + r[8]^2) + c_1^4*m^2)
    end
...
begin
	plot([r[1] for r in rs], [r[2] for r in rs], label="x", xlabel="t", ylabel="x")
	plot!([r[1] for r in rs], [r[3] for r in rs], label="y", xlabel="t", ylabel="y")
	plot!([r[1] for r in rs], [r[4] for r in rs], label="z", xlabel="t", ylabel="z")
end

# ╔═╡ 27cdddc0-525a-40e7-a287-e4b087337059
begin
	plot([r[1] for r in rs], [r[5] for r in rs], label="E", xlabel="t", ylabel="E")
end

# ╔═╡ 3cf9e9f8-6b3c-4f11-8105-82041e2ed2b5
begin
	plot([r[2] for r in rs], [r[3] for r in rs], label="(x,y)", xlabel="x", ylabel="y", aspect_ratio=:equal)
end

# ╔═╡ 2596b231-8d3f-4587-b5f9-5ef02927476b
begin
	plot([r[2] for r in rs], [r[3] for r in rs], [r[4] for r in rs], label="Trajectory", xlabel="x", ylabel="y", zlabel="z")
end
...
begin
	...
	cdf_values = [sum(bin_contents[1:i]) for i in 1:length(bin_positions)]
...
begin
	...
	histogram(bin_positions, weights=accepted_samples_hm, bins=62, xlabel="Mass (MeV/c²)", ylabel="Number", title="Histogram of Accepted Samples Hit and Miss", legend=false)
end
...
begin
	...
	### Plot histogram with the simulated sample
	histogram(bin_positions, weights=accepted_samples_inv, bins=62, xlabel="Mass (MeV/c²)", ylabel="Number", title="Histogram of Accepted Samples with Inverse CDF", legend=false)
end
...
function chi2(sample, data)
	...
	chi2_values = (sample .- data).^2 ./data
    sum(chi2_values)
end

# ╔═╡ b0b544b1-e26c-49d8-87cf-1002142534c7
# chi2_hm = ### Calculate chi2 of Hit and miss method
chi2_hm = chi2(accepted_samples_hm/sum(accepted_samples_hm), bin_contents/sum(bin_contents))

# ╔═╡ 7c274fb6-ccfe-484e-9c5b-321225186cb6
# chi2_inv = ### Calculate chi2 of Inverse CDF method
chi2_inv = chi2(accepted_samples_inv/sum(accepted_samples_inv), bin_contents/sum(bin_contents))
...
if eff_hm > eff_inv
	println("Hit or miss is more effective")
else
	println("Inverse transform sampling is more effective")
end
...
function momentum_from_circle(point1, point2, q, B)
	...
	# uconvert(u"GeV/c", p) 	# not working
...
# Function to calculate the radius of the circle given two state vectors. DONT USE FOR RANDOM MOMENTA
...
# Function to compute momentum based on the radius of the circle through two points. DONT USE FOR RANDOM MOMENTA 
...
begin
	...
	M_mumu = sample_values(cdf_values, bin_positions, Nevents)
	...
	for M in M_mumu
		...
		# decay of J/psi into two muons
		p_minus, p_plus = jpsi_to_mumu(M)
		
		
		# get the absolut value of the initial momenta
		p_initial_minus = sqrt(p_minus[2]^2 + p_minus[3]^2 + p_minus[4]^2)
		p_initial_plus = sqrt(p_plus[2]^2 + p_plus[3]^2 + p_plus[4]^2)
		
		
		push!(pmu_minus, copy(p_initial_minus))
		push!(pmu_plus, copy(p_initial_plus))


		# simulate trajectory with Runge-Kutta method
		r0_minus = append!([tmu0, xmu0, ymu0, zmu0],copy(p_minus))
		r0_plus = append!([tmu0, xmu0, ymu0, zmu0],copy(p_plus))
		
		rs_minus = runge_kutta_4(eom, r0_minus, par_minus, tmumax, dtmu)
		rs_plus = runge_kutta_4(eom, r0_plus, par_plus, tmumax, dtmu)		


		# calculate radius and reconstruct momenta based on trajectory. Must use function momentum_from_circle since the combination of the functions radius_of_circle and momentum_from_radius will return wrong results if the components of the momenta in the direction of the magnetic field is not 0.
		rad_minus, p_recon_minus = momentum_from_circle(rs_minus[1], rs_minus[2], par_minus[1], par_minus[4])
		rad_plus, p_recon_plus = momentum_from_circle(rs_plus[1], rs_plus[2], par_plus[1], par_plus[4])

		push!(R_minus, copy(rad_minus))
		push!(R_plus, copy(rad_plus))
		push!(pmu_minus_sim, copy(p_recon_minus))
		push!(pmu_plus_sim, copy(p_recon_plus))

		
		# calculate error
		push!(error_minus, abs(p_initial_minus - p_recon_minus)/p_initial_minus)
		push!(error_plus, abs(p_initial_plus - p_recon_plus)/p_initial_plus)
	end
	println("R_minus: ", R_minus)
	println("pmu_minus: ", pmu_minus)
	println("pmu_minus_sim: ", pmu_minus_sim)
	println("error_minus: ", error_minus)
	println("R_plus: ", R_plus)
	println("pmu_plus: ", pmu_plus)
	println("pmu_plus_sim: ", pmu_plus_sim)
	println("error_plus:", error_plus)
end
